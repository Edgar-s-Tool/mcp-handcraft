...
TOOLS = [
    {
        ...
        "inputSchema": {"type": "object", "properties": {"path": {"type": "string", "description": "Relative path inside the vault, e.g. 'Daily Notes/2026-04-04.md'"}}, "required": ["path"]},
    },
    {
        "name": "vault_write",
    ...
    {
        "name": "vault_list",
        "description": "List notes and folders in the Obsidian vault.",
        "inputSchema": {"type": "object", "properties": {"folder": {"type": "string", "description": "Subfolder to list; leave empty for root"}}},
    },
    {
        "name": "vault_search",
        "description": "Full-text search across all Obsidian vault notes.",
        "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "max_results": {"type": "integer"}}, "required": ["query"]},
    },
    {
        "name": "vault_today",
        "description": "Get or create today's Daily Note in the Obsidian vault.",
        "inputSchema": {"type": "object", "properties": {}},
    },
    {
        "name": "vault_delete",
        "description": "Delete a note or folder from the Obsidian vault.",
        "inputSchema": {"type": "object", "properties": {"path": {"type": "string", "description": "Relative path inside the vault to delete"}}, "required": ["path"]},
    },
    {
        "name": "vault_move",
        "description": "Move or rename a note or folder inside the Obsidian vault.",
        "inputSchema": {"type": "object", "properties": {"src": {"type": "string", "description": "Source relative path"}, "dst": {"type": "string", "description": "Destination relative path"}}, "required": ["src", "dst"]},
    },
    {
        "name": "echo",
...
def normalize_vault_relative_path(path: str) -> tuple[str | None, str | None]:
    raw = (path or "").strip()
    if not raw:
...
def resolve_vault_path(path: str) -> tuple[str | None, str | None, str | None]:
...
def vault_read(path: str) -> tuple[str, bool]:
...
def vault_write(path: str, content: str) -> tuple[str, bool]:
    rel, full, error = resolve_vault_path(path)
    if error:
        return error, True
...
def vault_list(folder: str = "") -> tuple[str, bool]:
    if folder:
    ...
    if not os.path.isdir(base):
        return f"Not found: {folder}", True
    lines = [
        ("📁 " if e.is_dir() else "📄 ") + e.name
        for e in sorted(os.scandir(base), key=lambda e: (not e.is_dir(), e.name.lower()))
        if not e.name.startswith(".")
    ]
    return "\n".join(lines) or "(empty)", False


def vault_search(query: str, max_results: int = 10) -> tuple[str, bool]:
    q = query.lower()
    results = []
...
def vault_today() -> tuple[str, bool]:
    ...
    if os.path.isfile(tmpl):
        ...
        content = content.replace('<% tp.date.now("YYYY-MM-DD") %>', today)
        content = content.replace('<% tp.date.now("YYYY-MM-DD dddd") %>', today)
    else:
        content = f"---\ndate: {today}\ntags: [daily]\n---\n\n# {today}\n\n## 今日重點\n-\n\n## 任務\n- [ ]\n"
...
def vault_delete(path: str) -> tuple[str, bool]:
    full = os.path.join(VAULT_PATH, path.strip().lstrip("/\\"))
...
def vault_move(src: str, dst: str) -> tuple[str, bool]:
    full_src = os.path.join(VAULT_PATH, src.strip().lstrip("/\\"))
    full_dst = os.path.join(VAULT_PATH, dst.strip().lstrip("/\\"))
    if not os.path.exists(full_src):
        return f"Not found: {src}", True
...
def handle_tools_call(req_id, params: dict) -> dict:
    ...
    if name == "vault_read":
        text, is_error = vault_read(arguments.get("path", ""))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))

    if name == "vault_write":
        text, is_error = vault_write(arguments.get("path", ""), arguments.get("content", ""))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))

    if name == "vault_move":
        text, is_error = vault_move(arguments.get("src", ""), arguments.get("dst", ""))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))

    if name == "vault_delete":
        text, is_error = vault_delete(arguments.get("path", ""))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))

    if name == "vault_list":
        text, is_error = vault_list(arguments.get("folder", ""))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))

    if name == "vault_search":
        text, is_error = vault_search(arguments.get("query", ""), arguments.get("max_results", 10))
        return make_response(req_id, make_tool_text_response(text, is_error=is_error))
